...
const addProductFunc = (state, action) => {
    ...
    state.basketProducts.push(added)
    state.totalPrice = +(state.totalPrice + addedTotalPrice).toFixed(2)
    state.totalProductsCount = state.totalProductsCount + 1
};
const updateProductFunc = (state, action) => {
    const {id, newCount} = action.payload;
    const productIndex = state.basketProducts.findIndex((p) => p.id === id);
    const product = state.basketProducts[productIndex];
    const oldTotalPrice = product.totalPrice;
    const updateTotalPrice = +(newCount * product.price).toFixed(2);


    state.basketProducts = state.basketProducts.map((p) =>
        p.id === id
            ? { ...p, count: newCount, totalPrice: updateTotalPrice }
            : p
    )
    state.totalPrice = +((state.totalPrice - oldTotalPrice) + updateTotalPrice).toFixed(2)
};

const removeProductFunc = (state, action) => {
    const removed = state.basketProducts.find((p) => p.id === action.payload.id);

    state.basketProducts = state.basketProducts.filter(
        (p) => p.id !== action.payload.id
    );
    state.totalPrice = +(state.totalPrice - removed.totalPrice).toFixed(2);
    state.totalProductsCount = state.totalProductsCount - 1;
};
